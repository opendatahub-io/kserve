...
The overlay uses a pluggable cert-manager architecture with RHAII-scoped CA
(can be shared by other RHAII components):

```
rhaii-selfsigned-issuer (ClusterIssuer)
    └── rhaii-ca (Certificate in cert-manager namespace)
            └── rhaii-ca-issuer (ClusterIssuer using the CA)
...
- `rhaii-selfsigned-issuer`: Bootstrap ClusterIssuer (replace with Vault/Let's Encrypt for production)
- `rhaii-ca`: CA certificate in `cert-manager` namespace
- `rhaii-ca-issuer`: ClusterIssuer that signs workload certificates
