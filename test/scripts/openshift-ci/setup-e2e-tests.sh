...
kubectl create -f config/overlays/odh-test/dsci.yaml
kubectl create -f config/overlays/odh-test/dsc.yaml
...
if [[ "${MARKERS}" == *"raw"* ]]; then
  oc patch configmap inferenceservice-config -n ${NS} --patch-file <(cat config/overlays/odh/test/configmap/inferenceservice-openshift-ci-raw.yaml | envsubst)
...
if [[ "${MARKERS}" == *"graph"* ]]; then
    oc patch configmap inferenceservice-config -n ${NS} --patch-file <(cat config/overlays/odh/test/configmap/inferenceservice-openshift-ci-serverless.yaml | envsubst)
  else 
    oc patch configmap inferenceservice-config -n ${NS} --patch-file <(cat config/overlays/odh/test/configmap/inferenceservice-openshift-ci-serverless-predictor.yaml | envsubst)
fi
