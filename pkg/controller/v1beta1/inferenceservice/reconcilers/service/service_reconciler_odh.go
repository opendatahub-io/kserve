...
func buildServiceList(componentMeta metav1.ObjectMeta, componentExt *v1beta1.ComponentExtensionSpec,
	...
	// Ensure the first port has a name so the OAuth port replacement logic can
	// consistently swap it by position when auth is enabled.
	if len(svcList[0].Spec.Ports) > 0 && len(svcList[0].Spec.Ports[0].Name) == 0 {
		svcList[0].Spec.Ports[0].Name = "http"
	}
