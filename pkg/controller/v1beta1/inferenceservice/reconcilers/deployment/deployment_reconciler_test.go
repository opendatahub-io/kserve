...
func TestExistingRawDeploymentWithAuthDisabled_NoOAuthProxyAdded(t *testing.T) {
	// Simulate existing deployment without OAuth proxy
	...
	client := &mockClientForCheckDeploymentExist{
		getDeployment: existingDeployment, // Deployment exists
		getErr:        nil,
	}
	clientset := fake.NewSimpleClientset(&corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{Name: constants.InferenceServiceConfigMapName, Namespace: constants.KServeNamespace},
	...
	// Verify OAuth proxy is NOT added to existing deployment
	containers := deployments[0].Spec.Template.Spec.Containers
	oauthProxyFound := false
	for _, container := range containers {
		if container.Name == constants.KubeRbacContainerName {
...
func TestExistingRawDeploymentWithAuthEnabled_PreservesOAuthProxy(t *testing.T) {
	// Simulate existing deployment WITH OAuth proxy
	existingDeployment := &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
	...
	// Verify OAuth proxy is still present
	containers := deployments[0].Spec.Template.Spec.Containers
	oauthProxyFound := false
	for _, container := range containers {
		if container.Name == constants.KubeRbacContainerName {
...
func TestNewInferenceGraph_NoOAuthProxy(t *testing.T) {
	...
	deployments, err := createRawDeploymentODH(
		...
		client,
		clientset,
		constants.InferenceGraphResource, // InferenceGraph resource type
		objectMeta,
		metav1.ObjectMeta{},
		&v1beta1.ComponentExtensionSpec{},
		&corev1.PodSpec{},
