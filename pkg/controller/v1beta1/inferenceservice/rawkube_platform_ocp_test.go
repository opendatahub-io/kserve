...
// Shared test helpers for cluster-local ISVCs (no Route created in distro mode).

func assertPlatformIngressExists(_ context.Context, _ types.NamespacedName) {}

func assertPlatformIngressDoesNotExist(_ context.Context, _ types.NamespacedName) {}

func assertPlatformIngressDeleted(_ context.Context, _ types.NamespacedName) {}

func assertPlatformIngressSpec(_ context.Context, _ types.NamespacedName, _ string, _ *v1beta1.InferenceService) {
}
...
func expectedIsvcStatusURL(serviceName, namespace string) (string, string) {
	predictorName := constants.PredictorServiceName(serviceName)
	return "http", fmt.Sprintf("%s.%s.svc.cluster.local", predictorName, namespace)
}

func simulateAuthRouteAdmission(ctx context.Context, serviceKey types.NamespacedName, serviceName string) {
	route := &routev1.Route{}
	Eventually(func() error {
		return k8sClient.Get(ctx, serviceKey, route)
	}, timeout, interval).Should(Succeed())

	admittedHost := fmt.Sprintf("%s-%s.%s", serviceName, serviceKey.Namespace, domain)
...
var _ = Context("When creating an inferenceservice with raw kube predictor and ODH auth enabled", func() {
  ...
  resourceAttributes:
    ...
    verb: "get"`, serviceKey.Namespace, serviceKey.Name),
		...
		simulateAuthRouteAdmission(ctx, serviceKey, serviceName)
...
var _ = Context("When kube-rbac-proxy image changes in config", func() {
