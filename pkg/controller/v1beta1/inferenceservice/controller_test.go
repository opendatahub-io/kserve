...
var _ = Describe("v1beta1 inference service controller", func() {
	...
	Context("When creating an inference service with a unsupported storageURI", func() {
		It("Should fail if unsupported storageURI is set", func() {
			...
			serviceName := "foo-1"
			var expectedRequest = reconcile.Request{NamespacedName: types.NamespacedName{Name: serviceName, Namespace: defaultNamespace}}
			var serviceKey = expectedRequest.NamespacedName
			var storageUri = "sss3://test/mnist/export"
			ctx := context.Background()
			isvc := &v1beta1.InferenceService{
				ObjectMeta: metav1.ObjectMeta{
					Name:      serviceKey.Name,
			...
			lastFailureInfo := v1beta1.FailureInfo{
				...
				Message: fmt.Sprintf(v1beta1.UnsupportedStorageURIFormatError, strings.Join(constants.SupportedStorageURIPrefixList, ", "), storageUri),
			}
