...
var _ = Describe("v1beta1 inference service controller", func() {
	...
	Context("When creating inference service with predictor", func() {
		...
		It("Should create a knative service with initial-scale:0 and min-scale:0 annotations", func() {
			...
			_, ok := actualService.Spec.Template.Annotations[constants.MaxScaleAnnotationKey]
			Expect(ok).To(Equal(false))
		})
		It("Should create a knative service with min-scale:0 and max-scale:1 annotations", func() {
			By("Configuring knative to disallow zero initial scale and requesting 0 min replicas and 1 max replicas for the inference service")
			...
			Expect(actualService.Spec.Template.Annotations[constants.MinScaleAnnotationKey]).To(Equal("0"))
			Expect(actualService.Spec.Template.Annotations[constants.MaxScaleAnnotationKey]).To(Equal("1"))
			_, ok := actualService.Spec.Template.Annotations[constants.InitialScaleAnnotationKey]
			Expect(ok).To(Equal(false))
		})
		It("Should create a knative service with initial-scale:0, min-scale:0, max-scale:1 annotations", func() {
			By("Configuring knative to allow zero initial scale and requesting 0 min replicas and 1 max replicas for the inference service")
			// Create the config-autoscaler configmap with allow-zero-initial-scale set to false
			var autoscalerConfigMap = &v1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      constants.DefaultAutoscalerConfigName,
