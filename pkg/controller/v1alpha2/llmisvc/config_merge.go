...
//
// vLLMTLSProfile was deleted rather than retired, which is sound only because it
// never reached a release. An entry that has must be frozen.
...
//
// It must be the whole entry - buried in a longer string it reaches the container.
// Presets older than the marker cannot contain it, so they render unchanged.
const OmittedArgMarker = "__KSERVE_OMIT_ARG__"

// dropOmittedArgs removes marked entries from the command and args of every
// container in every pod spec. Only those fields: a marker in a probe or an env
// value would survive, which is why the marker's contract is argv entries.
func dropOmittedArgs(cfg *v1alpha2.LLMInferenceServiceConfig) {
	...
	isMarker := func(arg string) bool { return arg == OmittedArgMarker }
	for _, podSpec := range podSpecs {
		...
		for _, containers := range [][]corev1.Container{podSpec.InitContainers, podSpec.Containers} {
			for i := range containers {
				containers[i].Command = slices.DeleteFunc(containers[i].Command, isMarker)
				containers[i].Args = slices.DeleteFunc(containers[i].Args, isMarker)
			}
		}
	}
}
