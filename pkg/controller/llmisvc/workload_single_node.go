...
func (r *LLMInferenceServiceReconciler) reconcileSingleNodeWorkload(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService) error {
	...
	if err := r.reconcileSingleNodeMainServiceAccount(ctx, llmSvc); err != nil {
...
func (r *LLMInferenceServiceReconciler) expectedSingleNodeMainDeployment(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService) (*appsv1.Deployment, error) {
	...
	if llmSvc.Spec.Template != nil {
		...
		if hasRoutingSidecar(d.Spec.Template.Spec) {
			...
			// TODO use function to get InferencePool name
			s.Env = append(s.Env, corev1.EnvVar{
				Name:      "INFERENCE_POOL_NAME",
				Value:     kmeta.ChildName(llmSvc.GetName(), "-inference-pool"),
				ValueFrom: nil,
			})
		}
	}
...
func (r *LLMInferenceServiceReconciler) reconcileSingleNodeMainServiceAccount(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService) error {
	expectedDeployment, err := r.expectedSingleNodeMainDeployment(ctx, llmSvc)
	if err != nil {
		return fmt.Errorf("failed to get expected main deployment: %w", err)
	}
	...
	if err := r.reconcileSingleNodeMainRole(ctx, llmSvc); err != nil {
		return err
	}

	return r.reconcileSingleNodeMainRoleBinding(ctx, llmSvc, serviceAccount)
}

func (r *LLMInferenceServiceReconciler) reconcileSingleNodeMainRole(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService) error {
	expectedDeployment, err := r.expectedSingleNodeMainDeployment(ctx, llmSvc)
	if err != nil {
		return fmt.Errorf("failed to get expected main deployment: %w", err)
	}
...
func (r *LLMInferenceServiceReconciler) reconcileSingleNodeMainRoleBinding(ctx context.Context, llmSvc *v1alpha1.LLMInferenceService, sa *corev1.ServiceAccount) error {
	expectedDeployment, err := r.expectedSingleNodeMainDeployment(ctx, llmSvc)
	if err != nil {
		return fmt.Errorf("failed to get expected main deployment: %w", err)
	}
