...
def prepare_env_and_deploy(args):
    ...
    if args["action"] == "delete":
        ...
        result = subprocess.run(['oc', 'project'], capture_output=True, text=True, check=True)
        ...
        if result.returncode == 0:
            ...
            subprocess.run(f"oc new-project {args['namespace']}", shell=True, check=True)
...
def _handle_create_action(args):
    ...
    if args['action'] == "create":
        print(f"Checking if InferenceService '{args['model_name']}' exists in namespace '{args['namespace']}'...")
        ...
        if result.returncode == 0:
            print(f"InferenceService '{args['model_name']}' already exists. Switching action to 'update'.")
            args['action'] = "update"
        else:
            print(f"InferenceService '{args['model_name']}' does not exist. Proceeding with 'create'.")
    ...
    if args['action'] == "update":
        ...
        if result.returncode != 0:
            print(f"InferenceService '{args['model_name']}' does not exist. Switching action to 'apply'.")
            args.action = "apply"
        else:
            print(f"InferenceService '{args['model_name']}' exists. Proceeding with 'update'.")
...
def compile_pipeline(action):
    ...
    if action == "delete":
        subprocess.run("kfp dsl compile --py pipeline/delete_model_pipeline.py --output pipeline.yaml", shell=True, check=True)
        return
    subprocess.run("kfp dsl compile --py pipeline/deploy_model_pipeline.py --output pipeline.yaml", shell=True, check=True)
