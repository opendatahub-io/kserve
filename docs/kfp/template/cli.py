...
def get_parsed_args():
    ...
    # Optional args
    parser.add_argument('--runtime_version', default='latest', help="Runtime Version of ML Framework")
    parser.add_argument('--resource_requests', default='{"cpu": "0.5", "memory": "512Mi"}',
                        help="CPU and Memory requests")
    parser.add_argument('--resource_limits', default='{"cpu": "1", "memory": "1Gi"}',
                        help="CPU and Memory limits")
    parser.add_argument('--custom_model_spec', default='{}', help="Custom model runtime container spec in JSON")
    parser.add_argument('--autoscaling_target', default='0', help="Autoscaling Target Number")
    parser.add_argument('--service_account', default='', help="ServiceAccount to use")
    parser.add_argument('--enable_istio_sidecar', type=bool, default=True,
                        help="Enable istio sidecar injection")
    parser.add_argument('--inferenceservice_yaml', default='{}',
    ...
    if args.action != "delete":
        ...
        if missing:
            parser.error(f"The following arguments are required when action != delete: {', '.join(missing)}")
