...
- `config/rbac/role.yaml` - generated by controller-gen; must not contain OCP-specific API groups.
  If `openshift.io` or `opendatahub.io` entries appear, it means a `//+kubebuilder:rbac:` marker
  was placed outside the `distro/` sub-package - see `rbac-isolation.md`.
